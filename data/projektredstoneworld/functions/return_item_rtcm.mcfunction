...
execute store result score #rtcmslot info run data get entity @s data.Inventory[0].Slot
...
scoreboard players remove #restorertcmitems info 1
# run the same function again if there are more items to process
execute if score #restorertcmitems info matches 1.. run function projektredstoneworld:return_item_rtcm
