...
execute as @p[tag=iceboat_time_tracked] run tellraw @a [{"text":"[IceBoat] ","color":"dark_aqua"},{"selector":"@p[tag=iceboat_time_tracked]"},{"text":" has finished the track in roughly "},{"score":{"name":"@s","objective":"iceboat_time"}}, {"text":" seconds! (exactly "},{"score":{"name":"@s","objective":"iceboat_ticks"}},{"text":" ticks)"}]
