...
execute as @p[tag=iceboat_time_tracked] store result storage iceboat:time_result seconds float .05 run scoreboard players get @s iceboat_tick
# The message is open to amendment
tellraw @a [{"selector":"@p[tag=iceboat_time_tracked]"},{"text":" has finished the track in"},{"storage":"iceboat:time_result","nbt":"seconds"}, {"text":" seconds!"}]
